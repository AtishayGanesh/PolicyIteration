function [probability,reward] = probab(s1,s2,c1,c2,chg,req1,req2,ret1,ret2)
...
for i = max(0,t1-c1):t1
   ...
   returned = i+c1-t1;
   if c1 ==20
       curr_prob = req1(rented+1)*sum(ret1(returned+1:21));
   else
...
if c1 ==20
    curr_prob = sum(req1(t1+2:21))*sum(ret1(c1+1:21));
else
    curr_prob = sum(req1(t1+2:21))*ret1(c1+1);
end
prob = prob + curr_prob;
...
for j =max(0,t2-c2):t2
   ...
   returned = j+c2-t2;
   if c2==20
       curr_prob = req2(rented+1)*sum(ret2(returned+1:21));
   else
...
if c2==20
    curr_prob = sum(req2(t2+2:21))*sum(ret2(c2+1:21));
else
    curr_prob = sum(req2(t2+2:21))*ret2(c2+1);
end
prob2 = prob2 + curr_prob;
